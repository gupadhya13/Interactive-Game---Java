...
public class TwisterView extends GameView {
	...
	void refreshGameRoundView(GameRound gameRound) {
		...
		for (int i = 0; i < gameRound.getClueWord().length(); i++) {
			clueButtons[i] = new Button(Character.toString(gameRound.getClueWord().charAt(i)));
			clueButtons[i].setPrefSize(70, 70);
			clueButtons[i].setStyle(
					" -fx-background-insets: 0,1,2;" + " -fx-font: 20px Arial;" + " -fx-background-color: lightblue;"
		...
		for (int i = 0; i < gameRound.getClueWord().length(); i++) {
			...
			answerButtons[i].setStyle(
					" -fx-background-insets: 0,1,2;" + " -fx-font: 20px Arial;" + " -fx-background-color: lightgray;"
							+ " -fx-background-insets: 0,1,1;" + "-fx-background-radius: 2em;");
			answerButtons[i]
					.setBackground(new Background(new BackgroundFill(Color.WHITE, CornerRadii.EMPTY, Insets.EMPTY)));
