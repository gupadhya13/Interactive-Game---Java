...
public class HangmanView extends GameView {
	...
	void setupSizesAlignmentsEtc() {
		...
		newWordButton.setStyle("-fx-background-color: gray," + " linear-gradient(lightblue, gray), "
		...
		scoreLabel.setStyle("-fx-background-color: lightblue; " + "-fx-font: 20px Arial; ");
		scoreLabel.setAlignment(Pos.CENTER);

		for (int i = 0; i < 26; i++) {
			keyboardButtons[i].setPrefSize(100, 50);
			keyboardButtons[i].setStyle("-fx-background-color: lightblue," + " linear-gradient(white, blue),"
					+ " linear-gradient(white 0%, white 49%, white 50%, lightblue 100%);" + " -fx-font: 20px Arial;"
	...
	/**
	 * refreshGameRoundView() clears up previous game round and prepares the
	 * clueGrid with new clue labels using the new clueWord. It also refreshes the
	 * keyboardButtons, disabling those buttons that have letters in the clue
	 */
	@Override
	void refreshGameRoundView(GameRound hangmanRound) {
		...
		for (int i = 0; i < hangmanRound.getClueWord().length(); i++) {
			...
			clueLabels[i].setStyle(
					" -fx-background-insets: 0,1,2;" + " -fx-font: 30px Arial;" + " -fx-background-color: white;"
							+ " -fx-background-insets: 0,1,1;" + "-fx-background-radius: 2em;");
			clueLabels[i]
		...
		for (int i = 0; i < keyboardButtons.length; i++) {
			if (hangmanRound.getClueWord().contains(keyboardButtons[i].getText()))
				...
				keyboardButtons[i].setDisable(false);
		}
	}
