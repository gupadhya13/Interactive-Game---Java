...
public abstract class GameView {
	...
	/**
	 * GameView() sets up the parts common to both Hangman and Twister. It creates
	 * the main grids, topMessageText and sets up the smileyButton.
	 */
	GameView() {
	...
	/**
	 * setupSmileyButton() initializes all smiley buttons with their images, sizes,
	 * styles, etc.
	 */
	void setupSmileyButton() {
		...
		smileyImageViews[SMILEY_INDEX]
				.setImage(new Image(getClass().getClassLoader().getResource("smiley.png").toString()));
		smileyImageViews[THUMBS_UP_INDEX]
				.setImage(new Image(getClass().getClassLoader().getResource("thumbsup.png").toString()));
		smileyImageViews[THUMBS_DOWN_INDEX]
				.setImage(new Image(getClass().getClassLoader().getResource("thumbsdown.png").toString()));
		smileyImageViews[REPEAT_INDEX]
				.setImage(new Image(getClass().getClassLoader().getResource("repeat.png").toString()));
		smileyImageViews[SADLY_INDEX]
				.setImage(new Image(getClass().getClassLoader().getResource("sadly.jpg").toString()));

		smileyButton.setStyle(" -fx-background-insets: 0,1,1;" + " -fx-padding: 3 3 3 3;"
				+ " -fx-background-radius: 5em; " + " -fx-background-color: white; "
